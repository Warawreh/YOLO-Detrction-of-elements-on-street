...
def classToName(x):
    to = ['GRAFFITI','FADED_SIGNAGE','POTHOLES','GARBAGE','CONSTRUCTION_ROAD','BROKEN_SIGNAGE','BAD_STREETLIGHT','BAD_BILLBOARD','SAND_ON_ROAD','CLUTTER_SIDEWALK','UNKEPT_FACADE']
    return to[x]
...
def detect_image(Yolo, image_path, output_path, input_size=416, show=False, CLASSES=YOLO_COCO_CLASSES, score_threshold=0.3, iou_threshold=0.45, rectangle_colors=''):
    ...
    if(len(bboxes) == 0):
        ...
        #print(coords , classToName(cls)) 
        with open(output_path, "a") as f:
            f.write(str(cls) + ',' + newPath + ',' + classToName(cls) + ',' + str(coords[2]) + ',' + str(coords[0]) + ',' + str(coords[3]) + ',' + str(coords[1]) + '\n')
    else:
